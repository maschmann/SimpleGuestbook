        ...
        <li class="$EvenOdd $FirstLast">
            ...
            <p class="smallText name">
                ...
                <% if Url %>&nbsp;<a href="$Url.XML" class="homepageLink" title="<% _t('Guestbook.ss.HOMEPAGE', 'website') %>">(<%
                _t('Guestbook.ss.HOMEPAGE', 'website') %>)</a><% end_if %>
            ...
            <p class="smallText entryTools">
                ...
                <a class="spam" href="{$Top.Link}doAction/$ID?do=spam" title="<% _t('Guestbook.ss.ENTRYISSPAM', 'entry is spam') %>">
                    &nbsp;</a>
                <% end_if %>
                <% if checkPermission(GUESTBOOK_DELETEENTRY) %>
                <a class="delete" href="{$Top.Link}doAction/$ID?do=deleteEntry" title="<% _t('Guestbook.ss.DELENTRY', 'delete entry') %>">
                    &nbsp;</a>
                <% end_if %>
                <% if Top.EntryComments %>
                <% if checkPermission(GUESTBOOK_ADDCOMMENT) %>
                <a href="{$Top.Link}doAction/$ID?do=addComment" class="comment popForm" title="<% _t('Guestbook.ss.NEWCOMMENT', 'add comment') %>">
                    &nbsp;</a>
                <% end_if %>
                <% end_if %>
            </p>
