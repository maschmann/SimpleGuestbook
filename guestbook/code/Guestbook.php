...
class Guestbook_Controller extends Page_Controller implements PermissionProvider
{
        ...
        if( is_object( $fields ) ) {
            ...
            if( class_exists( 'MathSpamProtection' )
                ...
                $fields->push(
                    new TextField( 'Math',
                        sprintf(
                            _t( 'Guestbook.SPAMQUESTION', "Spam protection question: %s" ),
                            MathSpamProtection::getMathQuestion()
                        )
                    )
                );
            }
    ...
    public function doSubmitEntry( $data, $form )
    {
        ...
        if( false == $blnIsBlocked ) {
            ...
            $form->sessionMessage(
                _t(
                    'Guestbook.ENTRYBLOCKED',
                    'This email is supposed to be spam-related. Please verify if it is correct'
                ),
                'error'
            );
        }
